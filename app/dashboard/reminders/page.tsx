...
function isClientActive(status: string) {
...
export default function RemindersPage() {
  ...
  const activeInvoices = (invoiceRows ?? []).filter(({ invoice }: any) => isClientActive(invoice.status));
  const overdueInvoices = (invoiceRows ?? []).filter(({ invoice }: any) => invoice.status === "overdue");

  async function handleReminder(invoice: any) {
