...
export default function ReportsPage() {
  ...
  const outstanding = rows.filter(({ invoice }) => ["sent","viewed","approved","awaiting_payment"].includes(invoice.status));
  ...
  const totalOutstanding = outstanding.reduce((s, { invoice }) => s + (invoice.amountTotal ?? invoice.amount ?? 0), 0);
  const totalOverdue = overdue.reduce((s, { invoice }) => s + (invoice.amountTotal ?? invoice.amount ?? 0), 0);
